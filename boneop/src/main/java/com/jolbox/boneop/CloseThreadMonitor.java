...
/**
 * Thread that monitors another thread and displays stack trace if
 * getConnection() was called without the corresponding close(). For debug
 * purposes only.
 *
 * @author Wallace
 *
...
public class CloseThreadMonitor<T> implements Runnable {
     ...
     */
    private ObjectHandle<T> connectionHandle;
    /**
     ...
     * @param closeConnectionWatchTimeout no of ms to wait in thread.join(). 0 =
     * wait forever
     */
    public CloseThreadMonitor(Thread threadToMonitor, ObjectHandle<T> connectionHandle, String stackTrace, long closeConnectionWatchTimeout) {
