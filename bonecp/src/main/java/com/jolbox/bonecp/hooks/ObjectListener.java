 ...
 * will generally want to extend
 * {@link com.jolbox.bonecp.hooks.AbstractConnectionHook} instead to provide a
...
public interface ObjectListener {
     ...
     * inserting into the pool). You may call connection.getInternalConnection()
     * to obtain a handle to the actual (unwrapped) connection obtained from the
     * driver.
     *
     * @param connection Handle to the new connection
     */
    void onAcquire(ObjectHandle connection);
     ...
     */
    void onCheckIn(ObjectHandle connection);
     ...
     */
    void onCheckOut(ObjectHandle connection);
     ...
     * has triggered an exception at some point. Connection has already been
     * closed by the time you receive this callback.
     *
     * @param connection
     */
    void onDestroy(ObjectHandle connection);

    /**
     * Called on attempting (and failing) to acquire a connection. Note that
