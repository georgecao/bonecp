...
import com.jolbox.boneop.proxy.TransactionRecoveryResult;
...
public class TestMemorizeTransactionProxy {
	...
	public void before(){
		...
		this.config =  new BoneOPConfig();
		this.config.setDisableObjectTracking(true);
	...
	public void testHSQLDB() throws ClassNotFoundException, SQLException{
		...
		this.config.setMinObjectsPerPartition(1);
		this.config.setMaxObjectsPerPartition(1);
...
//			e.printStackTrace();
	...
	public void testProxies() throws SQLException, SecurityException, IllegalArgumentException, NoSuchMethodException, NoSuchFieldException, IllegalAccessException{
		...
		InvocationHandler handler = java.lang.reflect.Proxy.getInvocationHandler( ((ObjectHandle) con).getInternalObject());
		...
		((ObjectHandle)con).getReplayLog().add(new ReplayLog(null, null, null));
		((ObjectHandle)con).recoveryResult.getReplaceTarget().put("test", "test1");
		con.rollback(); // should clear out log
		assertTrue(((ObjectHandle)con).getReplayLog().isEmpty());
...
//		assertTrue(((ConnectionHandle)con).recoveryResult.getReplaceTarget().isEmpty());
		...
		assertNotNull(((ObjectHandle)con).getProxyTarget());
		try{
			field = con.getClass().getDeclaredField("connection");
			field.setAccessible(true);
			field.set(con, null);
			((ObjectHandle)con).getProxyTarget();
	...
	public void testReplayTransaction() throws IllegalArgumentException, Throwable{
		...
		this.config.setMinObjectsPerPartition(2);
		this.config.setMaxObjectsPerPartition(2);
		BoneOP pool = new BoneOP(this.config);
...
//		.andThrow(new SQLException("just a fake exception for code coverage")).once();
		...
		((ObjectHandle)con).recoveryResult.getReplaceTarget().put(con, con);		// for code coverage
...
//		config.setConnectionHook(new CoverageHook());
	...
	public void testReplayTransactionWithFailuresCustomHook() throws IllegalArgumentException, Throwable{
		...
		this.config.setAcquireRetryDelayInMs(1);
		this.config.setAcquireRetryAttempts(2);
		this.config.setObjectListener(new CoverageHook());
		BoneOP pool = new BoneOP(this.config);
		reset(mockConnection);
		this.config.setTransactionRecoveryEnabled(false);
