...
public class TestXMLDefaultCreate {
	 ...
	 * @throws InvocationTargetException 
	 */
	private static Object fetchDefaultValue(BoneOPConfig config, String prop)
			throws SecurityException,
			IllegalArgumentException, IllegalAccessException, NoSuchFieldException, InvocationTargetException, NoSuchMethodException {
		Field field = null ;
	
		try{
			field = BoneOPConfig.class.getDeclaredField(prop);
		} catch (NoSuchFieldException e){
			return BoneOPConfig.class.getDeclaredMethod("get"+prop.substring(0, 1).toUpperCase()+prop.substring(1)).invoke(config);
		}
